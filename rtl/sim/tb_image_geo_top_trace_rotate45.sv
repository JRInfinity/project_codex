`timescale 1ns/1ps
// Keep tb_image_geo_top_trace_rotate45.md in sync
module tb_image_geo_top_trace_rotate45;
    ...
    localparam int TRACE_START = 320;
    ...
    task automatic program_case(input bit prefetch_en);
        begin
            ...
            axil_write(12'h014, {16'd48, 16'd48});
            axil_write(12'h018, {16'd32, 16'd32});
            ...
            if (wr_active_reg && m_axi_wr_wvalid && m_axi_wr_wready) begin
                ...
                for (byte_idx = 0; byte_idx < BYTE_W; byte_idx = byte_idx + 1) begin
                    if (m_axi_wr_wstrb[byte_idx]) begin
                        dst_mem[beat_addr + byte_idx] <= m_axi_wr_wdata[8*byte_idx +: 8];
                    end
                end
    ...
    always_ff @(posedge core_clk) begin
        ...
        if (!core_rstn) begin
            ...
            if (!trace_done_reg && dut.sample_req_valid && dut.sample_req_ready) begin
                ...
                if (trace_accept_count + 1 >= (TRACE_START + TRACE_LIMIT)) begin
                    trace_done_reg <= 1'b1;
                end
            end
    ...
    initial begin
        ...
        while (!trace_done_reg) begin
            ...
            if (cycle_count > 200000) begin
                $fatal(1, "Trace run timed out before collecting %0d accepted requests", TRACE_LIMIT);
            end
        end

`timescale 1ns/1ps
// Keep tb_image_geo_top_prefetch_stress.md in sync
module tb_image_geo_top_prefetch_stress;
        ...
        begin
            ...
            for (y = 0; y < 16; y = y + 1) begin
                for (x = 0; x < 64; x = x + 1) begin
                    src_mem[32'h0000_0100 + y*64 + x] = byte'(((y * 37) + (x * 11) + 5) & 8'hFF);
                end
            end
        end
    endtask
        ...
        begin
            while (!irq) begin
                ...
                if (cycle_count > 120000) begin
                    $fatal(1, "Stress top-level simulation timed out waiting for irq");
                end
            end
        end
    endtask

    task automatic check_status_done_ok(output logic [31:0] status_data);
        begin
            axil_read(12'h01C, status_data);
            if (!status_data[1]) begin
                $fatal(1, "Top-level stress status did not report done: 0x%08h", status_data);
            end
            if (status_data[2]) begin
                $fatal(1, "Top-level stress status reported error: 0x%08h", status_data);
            end
        end
    endtask
            ...
            for (y = 0; y < dst_h; y = y + 1) begin
                for (x = 0; x < dst_w; x = x + 1) begin
                    ...
                    if (dst_mem[dst_addr] !== expected) begin
                        $fatal(1, "Stress transform mismatch at dst(%0d,%0d): got=%0d exp=%0d",
    ...
    initial begin
        ...
        axil_write(12'h008, 32'h0000_2000);
        axil_write(12'h00C, 32'd64);
        axil_write(12'h010, 32'd64);
        axil_write(12'h014, {16'd16, 16'd64});
        axil_write(12'h018, {16'd16, 16'd64});
        ...
        check_transform_ref_generic(32'h0000_0100, 32'h0000_2000, 64, 64, 16, 64, 16, 32'sh0000_0000, 32'sh0001_0000);
        read_cache_stats(reads_no_prefetch, misses_no_prefetch, prefetches_no_prefetch, prefetch_hits_no_prefetch);
        $display("stress no-prefetch stats: reads=%0d misses=%0d prefetches=%0d hits=%0d",
            reads_no_prefetch, misses_no_prefetch, prefetches_no_prefetch, prefetch_hits_no_prefetch);
        if (reads_no_prefetch == 0 || misses_no_prefetch == 0) begin
            $fatal(1, "Prefetch-disabled stress run should report non-zero cache activity");
        end
        if (prefetches_no_prefetch != 0 || prefetch_hits_no_prefetch != 0) begin
            $fatal(1, "Prefetch-disabled stress run should report zero prefetch counters");
        end
        ...
        $display("stress prefetch stats: reads=%0d misses=%0d prefetches=%0d hits=%0d",
            reads_prefetch, misses_prefetch, prefetches_prefetch, prefetch_hits_prefetch);
        if (reads_prefetch == 0 || misses_prefetch == 0) begin
            $fatal(1, "Prefetch-enabled stress run should still report cache activity");
        end
        if (prefetches_prefetch == 0) begin
            $fatal(1, "Prefetch-enabled stress run should report non-zero prefetch starts");
        end
        if (misses_prefetch >= misses_no_prefetch) begin
            $fatal(1, "Prefetch-enabled stress run should reduce demand misses (%0d >= %0d)",
                misses_prefetch, misses_no_prefetch);
        end

        $display("tb_image_geo_top_prefetch_stress completed");
        $finish;
    end

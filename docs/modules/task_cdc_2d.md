...
> 依据文件：``rtl/axi/task_cdc_2d.sv``。文档结论来自源码、现有文档和可追溯文件名；不能确定处标为“待确认”。

## 1. 模块定位
- `task_cdc_2d` 是主链路基础模块：二维任务 CDC，传递 addr、row_stride、byte_count、row_count。
...
## 3. 主要功能
- 主要功能：二维任务 CDC，传递 addr、row_stride、byte_count、row_count。
...
- 时钟/复位：`src_clk`（input）。
- 时钟/复位：`src_rst`（input）。
- 握手/状态：`task_valid_src`（input）。
- 数据/控制：`task_base_addr_src`（input）。
- 数据/控制：`task_row_stride_src`（input）。
- 数据/控制：`task_row_byte_count_src`（input）。
- 数据/控制：`task_row_count_src`（input）。
- 握手/状态：`task_ready_src`（output）。
- 时钟/复位：`dst_clk`（input）。
- 时钟/复位：`dst_rst`（input）。
- 握手/状态：`task_valid_dst`（output）。
- 数据/控制：`task_base_addr_dst`（output）。
- 数据/控制：`task_row_stride_dst`（output）。
- 数据/控制：`task_row_byte_count_dst`（output）。
- 数据/控制：`task_row_count_dst`（output）。
- 握手/状态：`task_ready_dst`（input）。
...
> 本小节由 ``rtl/axi/task_cdc_2d.sv`` 中的声明和 ``always_ff`` 非阻塞赋值提取。表中的“变化条件”保留源码条件表达式名称，便于回查；未能从命名确定的语义标为“待确认”。

...
> 依据文件：``rtl/axi/ddr_axi_pkg.sv``。文档结论来自源码、现有文档和可追溯文件名；不能确定处标为“待确认”。

## 1. 模块定位
- `ddr_axi_pkg` 是主链路基础模块：AXI DDR 公共 package，提供 burst、4KB、word count 和 WSTRB 计算函数。
...
## 3. 主要功能
- 主要功能：AXI DDR 公共 package，提供 burst、4KB、word count 和 WSTRB 计算函数。
...
> 本小节由 ``rtl/axi/ddr_axi_pkg.sv`` 中的声明和 ``always_ff`` 非阻塞赋值提取。表中的“变化条件”保留源码条件表达式名称，便于回查；未能从命名确定的语义标为“待确认”。

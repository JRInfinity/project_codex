# tb_image_geo_top_perf_single_proxy512_rotate0_on

> 依据文件：``rtl/sim/tb_image_geo_top_perf_single_proxy512_rotate0_on.sv``。testbench/wrapper 不按可综合 RTL 同等深挖，仅记录验证目标、DUT、场景和日志追溯。

## 1. 模块定位
- testbench/wrapper，验证目标推断为 ``image_geo_top_perf_single_proxy512_rotate0_on``。
- 场景类型：性能/吞吐场景。

## 2. 文件路径
- ``rtl/sim/tb_image_geo_top_perf_single_proxy512_rotate0_on.sv``

...
> 依据文件：``rtl/axi/task_cdc.sv``。文档结论来自源码、现有文档和可追溯文件名；不能确定处标为“待确认”。

## 1. 模块定位
- `task_cdc` 是主链路基础模块：单段任务 CDC，打包地址和 byte_count，限制单任务 in-flight。
...
## 3. 主要功能
- 主要功能：单段任务 CDC，打包地址和 byte_count，限制单任务 in-flight。
...
- 数据/控制：`task_addr_src`（input）。
- 数据/控制：`task_byte_count_src`（input）。
- 握手/状态：`task_ready_src`（output）。
- 时钟/复位：`dst_clk`（input）。
- 时钟/复位：`dst_rst`（input）。
- 握手/状态：`task_valid_dst`（output）。
- 数据/控制：`task_addr_dst`（output）。
- 数据/控制：`task_byte_count_dst`（output）。
...
> 本小节由 ``rtl/axi/task_cdc.sv`` 中的声明和 ``always_ff`` 非阻塞赋值提取。表中的“变化条件”保留源码条件表达式名称，便于回查；未能从命名确定的语义标为“待确认”。
